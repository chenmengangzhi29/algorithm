...
int main(int argc, char *argv[])
{
    ...
    int checkpos = 0, i;
    while(1){
        //超时验证，每次测试100个连接，不测试listenfd当客户端60秒内没有和服务器通信，则关闭此客户端链接
        ...
        for(i = 0; i < 100; i++, checkpos++) {//一次循环检测100个。使用checkpos控制检测对象
            ...
            if(duration >= 60) {
                close(g_events[checkpos].fd);//关闭与客户端连接
